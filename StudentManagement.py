...
class StudentDatabaseApp:
    ...
    def create_widgets(self):
        # Heading
        heading = tk.Label(
            self.root, text="FACE RECOGNITION ATTENDANCE SYSTEM GUI", font=("Arial", 16)
        )
        ...
        self.attendance_menu = tk.OptionMenu(
            self.fields_frame, self.attendance_var, "A", "P", "E"
        )
        self.attendance_menu.grid(row=3, column=1, padx=5, pady=5)

        self.total_attendance_label = tk.Label(
            self.fields_frame, text="Total Attendance"
        )
        ...
        self.remove_button = tk.Button(
            self.root, text="Remove Student", command=self.remove_student
        )
        self.remove_button.pack(side=tk.LEFT, padx=10, pady=10)
        self.update_button = tk.Button(
            self.root, text="Update Student", command=self.update_student
        )
        self.update_button.pack(side=tk.LEFT, padx=10, pady=10)
        self.export_button = tk.Button(
            self.root, text="Export to Excel", command=self.export_to_excel
        )
        self.export_button.pack(side=tk.LEFT, padx=10, pady=10)
        self.reset_button = tk.Button(
            self.root, text="Reset Fields", command=self.reset_fields
        )
    ...
    def update_student(self):
        ...
        updated_data = {
            ...
            "total_attendance": (
                ...
                if total_attendance
                else current_data.get("total_attendance", 0)
            ),
            "last_attendance_time": (
                datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                if attendance in ["P", "E"]
                else current_data.get("last_attendance_time")
            ),
        }
